...
def test(model,topk):
    ...
    grad_cam = GradCam(model = model, target_layer_names = ["29"], use_cuda=True)
    ...
    for ordata, data, target in test_loader:
        ...
        for inum in range(len(pred)):
            ...
            if int(target[inum])==10:# no_mind keep for other use
                # original data
                ordata_D = ordata
		    ...
		    # print(savestream)
                    ...
                    plt.savefig(savestream+"/pred_"+arrayindex[int(pred[inum])]+"/"+str(64*batchindex+inum)+".png")
                    ...
                    plt.savefig(savestream+"/target_"+arrayindex[int(target[inum])]+"/"+str(64*batchindex+inum)+".png") 
                else:
                    if numoforginaldata[0,int(target[inum])]<=799:
                        savestream = (args.save+"/"+args.subject+"/pred_"+arrayindex[int(target[inum])]+"_"+arrayindex[int(pred[inum])])
		        # print(savestream)
